...
from synalinks.src.backend.pydantic.knowledge import Embedding
...
class EmbeddedEntityTest(testing.TestCase):
    ...
    def test_is_embedded_entity_true(self):
        ...
        self.assertTrue(is_embedded_entity(EmbeddedEntity(label="Doc", embedding=[1.0])))
...
class KnowledgeGraphsTest(testing.TestCase):
    def _kg(self):
    ...
    def test_knowledge_graphs_wraps_a_list(self):
        kgs = KnowledgeGraphs(knowledge_graphs=[self._kg(), self._kg()])
        self.assertEqual(len(kgs.knowledge_graphs), 2)
        self.assertEqual(len(kgs.knowledge_graphs[0].entities), 2)

    def test_is_knowledge_graphs_true(self):
        self.assertTrue(is_knowledge_graphs(KnowledgeGraphs))
