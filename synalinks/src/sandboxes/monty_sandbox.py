...
class MontySandbox(Sandbox, pydantic_monty.AbstractOS):
    ...
    description: str = (
        ...
        "third-party libraries, no `class` or `match` statements. For all "
        "file I/O use `pathlib.Path` — there is no `open()` builtin (e.g. "
        "read with `pathlib.Path(p).read_text()`, write with "
        "`pathlib.Path(p).write_text(s)`). A copy-on-write virtual "
        "filesystem is reachable via `pathlib.Path` (`read_text`, "
        "`write_text`, `read_bytes`, `write_bytes`, `exists`, `iterdir`, "
        "`unlink`, `mkdir`, ...); "
