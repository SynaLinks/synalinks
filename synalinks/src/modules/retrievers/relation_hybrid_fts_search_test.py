...
"""Tests for `RelationHybridFTSSearch`, focused on infer-the-label behavior.

The KB's relation enumeration (`get_symbolic_relations`) and search
(`relation_hybrid_fts_search`) are stubbed on a real KB instance, so the tests
...
class RelationHybridFTSSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.relation_hybrid_fts_search = _search
        return kb

    def test_infers_label_enum_from_kb(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = RelationHybridFTSSearch(knowledge_base=kb, language_model=lm, name="r")
