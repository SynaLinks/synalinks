...
"""Tests for `EntityHybridRegexSearch`, focused on the infer-the-label behavior.

The knowledge base's graph enumeration (`get_symbolic_entities`) and search
(`entity_hybrid_regex_search`) are stubbed on a real KB instance, so the tests
are deterministic and independent of the graph backend; only
``litellm.acompletion`` is mocked, to return the generated query (including the
inferred ``entity_label``).
"""
...
class EntityHybridRegexSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.entity_hybrid_regex_search = _search
        return kb

    def test_infers_label_enum_from_kb(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = EntityHybridRegexSearch(knowledge_base=kb, language_model=lm, name="e")
    ...
    async def test_call_uses_inferred_label(self):
        ...
        with patch(
            ...
            side_effect=_lm_returns(
                '{"similarity_search": ["alice"], "entity_label": "City"}'
            ),
        ):
