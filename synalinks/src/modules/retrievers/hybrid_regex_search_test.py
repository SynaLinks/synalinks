...
"""End-to-end tests for `HybridRegexSearch` against a real DuckDB store.

Exercises the real vector (HNSW) + RE2 regex fusion path. Docs carry
pre-computed embeddings (DuckDB does not embed on insert); only
...
from synalinks.src.modules.retrievers.hybrid_regex_search import HybridRegexSearch
...
class HybridRegexSearchTest(testing.TestCase):
    def setUp(self):
        super().setUp()
        self.temp_dir = tempfile.mkdtemp()
        self.db_path = "duckdb://" + os.path.join(self.temp_dir, "hybrgx.db")
    ...
    async def test_fuses_vector_and_regex(self):
        with patch("litellm.aembedding", side_effect=_fake_aembedding):
            ...
            with patch(
                ...
                side_effect=_lm_returns(
                    '{"similarity_search": ["fox"], "regex_patterns": ["fox"]}'
                ),
            ):
                inputs = Input(data_model=Query)
                outputs = await HybridRegexSearch(
                    knowledge_base=kb,
                    language_model=lm,
                    data_model=Document,
                    k=5,
        ...
        # both fox docs surface via vector and/or regex; dog doc ranks last/out
    ...
    async def test_none_input_returns_none(self):
        with patch("litellm.aembedding", side_effect=_fake_aembedding):
            kb = await self._kb()
            lm = LanguageModel(model="ollama/mistral")
            mod = HybridRegexSearch(
                knowledge_base=kb, language_model=lm, data_model=Document, name="hybrgx"
            )
    ...
    async def test_infers_table_name_when_not_provided(self):
        ...
        with patch("litellm.aembedding", side_effect=_fake_aembedding):
            ...
            lm = LanguageModel(model="ollama/mistral")
            mod = HybridRegexSearch(
                knowledge_base=kb, language_model=lm, k=5, name="hybrgx"
            )
