...
"""Tests for `PathSimilaritySearch`, focused on inferring the endpoint labels.
...
enumeration (`get_symbolic_entities`) and search (`path_similarity_search`) are
...
from synalinks.src.modules.retrievers.path_similarity_search import PathSimilaritySearch
...
class PathSimilaritySearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        async def _search(
            subj_queries, obj_queries, subj_label=None, obj_label=None, **kwargs
        ):
        ...
        kb.path_similarity_search = _search
        return kb

    def test_infers_both_endpoint_labels(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = PathSimilaritySearch(knowledge_base=kb, language_model=lm, name="p")
    ...
    async def test_call_uses_inferred_labels(self):
        ...
        with patch(
            ...
            side_effect=_lm_returns(
                '{"subj_similarity_search": ["alice"], '
                '"obj_similarity_search": ["paris"], '
