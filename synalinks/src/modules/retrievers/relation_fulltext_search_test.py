...
"""Tests for `RelationFullTextSearch`, focused on infer-the-label behavior.

The KB's relation enumeration (`get_symbolic_relations`) and search
(`relation_fulltext_search`) are stubbed on a real KB instance, so the tests
...
class RelationFullTextSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.relation_fulltext_search = _search
        return kb

    def test_infers_label_enum_from_kb(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = RelationFullTextSearch(knowledge_base=kb, language_model=lm, name="r")
    ...
    async def test_call_uses_inferred_label(self):
        ...
        with patch(
            ...
            side_effect=_lm_returns(
                '{"fulltext_search": ["lives"], "relation_label": "LivesIn"}'
            ),
        ):
