...
"""Tests for `PathHybridRegexSearch`, focused on inferring the endpoint labels.
...
enumeration (`get_symbolic_entities`) and search (`path_hybrid_regex_search`)
are stubbed on a real KB instance; only ``litellm.acompletion`` is mocked.
"""
...
class PathHybridRegexSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.path_hybrid_regex_search = _search
        return kb

    def test_infers_both_endpoint_labels(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = PathHybridRegexSearch(knowledge_base=kb, language_model=lm, name="p")
