...
"""Tests for `EntityHybridFTSSearch`, focused on the infer-the-label behavior.

The knowledge base's graph enumeration (`get_symbolic_entities`) and search
(`entity_hybrid_fts_search`) are stubbed on a real KB instance, so the tests are
...
class EntityHybridFTSSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.entity_hybrid_fts_search = _search
        return kb

    def test_infers_label_enum_from_kb(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = EntityHybridFTSSearch(knowledge_base=kb, language_model=lm, name="e")
