...
"""Tests for `PathHybridFTSSearch`, focused on inferring the endpoint labels.
...
enumeration (`get_symbolic_entities`) and search (`path_hybrid_fts_search`) are
...
from synalinks.src.modules.retrievers.path_hybrid_fts_search import PathHybridFTSSearch
...
class PathHybridFTSSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.path_hybrid_fts_search = _search
        return kb

    def test_infers_both_endpoint_labels(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = PathHybridFTSSearch(knowledge_base=kb, language_model=lm, name="p")
