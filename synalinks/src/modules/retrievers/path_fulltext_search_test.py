...
"""Tests for `PathFullTextSearch`, focused on inferring the endpoint labels.
...
enumeration (`get_symbolic_entities`) and search (`path_fulltext_search`) are
...
from synalinks.src.modules.retrievers.path_fulltext_search import PathFullTextSearch
...
class PathFullTextSearchInferTest(testing.TestCase):
    def setUp(self):
        super().setUp()
    ...
    def _kb(self, captured=None):
        ...
        kb.path_fulltext_search = _search
        return kb

    def test_infers_both_endpoint_labels(self):
        kb = self._kb()
        lm = LanguageModel(model="ollama/mistral")
        mod = PathFullTextSearch(knowledge_base=kb, language_model=lm, name="p")
    ...
    async def test_call_uses_inferred_labels(self):
        ...
        with patch(
            ...
            side_effect=_lm_returns(
                '{"subj_fulltext_search": ["alice"], '
                '"obj_fulltext_search": ["paris"], '
