...
- **手間いらずのモデル切り替え** : `synalinks.set_default_language_model(...)` で一度デフォルトを設定するか、文字列識別子を渡すだけで、[LiteLLM](https://docs.litellm.ai/docs/) 経由で Ollama、vLLM、OpenAI、Azure、Anthropic、Mistral、Groq、Gemini、xAI、Cohere、DeepSeek、Together AI、OpenRouter、AWS Bedrock、Doubleword、Mirai を切り替えられます。コストと品質の観点から最適なモデルを選ぶ[多目的モデル選択](https://synalinks.github.io/synalinks/guides/Multi-Objective%20LM%20Selection/)にも対応。
